...
def generate_compartment_door_frame(settings: CompartmentDoorSettings = CompartmentDoorSettings(), tolerances: CompartmentDoorTolerances = CompartmentDoorTolerances()) -> cq.Workplane:
    s = settings
    t = tolerances
    compartment_door = generate_compartment_door(settings, tolerances)
    compartment_door_with_tolerance = generate_compartment_door_with_tolerance(
        settings, tolerances)
    return (cq.Workplane(compartment_door_with_tolerance.faces("-Z", tag="compartment_door_with_tolerance").val())
            .faces()
            .wires()
            .toPending()
            .offset2D(-s.recessed_edge_width, "intersection")
            .extrude(-s.compartment_door_dimensions.z)
            .faces("|Z")
            .shell(s.recessed_edge_width, "intersection")
            )
...
if __name__ == "__main__":
    ...
    show_object(generate_compartment_door_frame(
        compartmentDoorSettings, compartmentDoorTolerances), name="compartment_door_frame")
